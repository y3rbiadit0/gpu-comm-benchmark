...
int main(int argc, char** argv) {
  ...
  try {
    ...
    for (int iteration = 0; iteration < iterations; ++iteration) {
      ...
      MPI_Sendrecv(in + 1U, halo_count, MPI_FLOAT, left, 0, in + local_size + 1U, halo_count,
                   MPI_FLOAT, right, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Sendrecv(in + local_size, halo_count, MPI_FLOAT, right, 1, in, halo_count, MPI_FLOAT,
                   left, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    if (rank == 0) {
      std::cout << "cuda_mpi_halo_1d_cuda_aware_iter n=" << global_size << " iterations=" << iterations
                << " ranks=" << ranks << " time_s=" << max_elapsed
