...
export COMM_PLAYGROUND_SYCL_FLAGS=${COMM_PLAYGROUND_SYCL_FLAGS:--fsycl --gcc-toolchain=${GCC12_ROOT} -fsycl-targets=${SYCL_TARGET} -Xsycl-target-backend=${SYCL_TARGET} --cuda-gpu-arch=${NVIDIA_GPU_ARCH}}
export SYCL_FLAGS="$COMM_PLAYGROUND_SYCL_FLAGS"
