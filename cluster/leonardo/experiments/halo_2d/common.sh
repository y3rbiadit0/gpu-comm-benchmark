...
cp_halo_2d_setup() {
  ...
  # halo_2d works on a square CP_N x CP_N grid; CP_N is the side length.
  CP_N=${CP_N:-4096}
  CP_ITERS=${CP_ITERS:-50}
  CP_WARMUP=${CP_WARMUP:-10}
  CP_NTRIALS=${CP_NTRIALS:-3}
  # halo_2d binaries accept: <side> [iterations] [warmup]
  CP_EXTRA_ARGS=${CP_EXTRA_ARGS:-"$CP_ITERS $CP_WARMUP"}
  CP_LAUNCHER=${CP_LAUNCHER:-srun}
  CP_RUN_DIR=${CP_RUN_DIR:-results/$CP_RESULT_NAME/halo_2d}

  [ -x "$CP_BINARY" ] || { echo "no executable: $CP_BINARY" >&2; exit 1; }
}

cp_halo_2d_print_summary() {
