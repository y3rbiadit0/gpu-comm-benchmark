...
def load_rows(path: Path):
    ...
    for name in (SM_HZ, DRAM_HZ, DRAM_BW, PEAK_DRAM, PEAK_DFMA, PEAK_FFMA):
        if name not in col:
            sys.exit(
                f"{path}: missing metric {name} -- was this exported with "
...
def main():
    ...
    ap.add_argument(
        ...
        "--output",
        type=Path,
        default=Path("roofline.svg"),
        help="output figure (.svg/.png/.pdf)",
    )
        ...
        for row, get in rows:
            ...
            for prec, rates in (("fp64", FP64_RATES), ("fp32", FP32_RATES)):
                ...
                if (
                    ...
                    > 0.005
                    * max(
                        sum(w * get(row, m) for m, w in r) * sm_hz for r in (FP64_RATES, FP32_RATES)
                    )
                    and flops > 0
                    and bw > 0
                ):
    ...
    for prec, style in (("fp64", "--"), ("fp32", "-")):
        roof = peaks[prec]
        ax.plot(xs, [min(ai * peaks["bw"], roof) / 1e9 for ai in xs], style, color="0.35", lw=1.2)
        ax.annotate(
            f"{prec.upper()} peak {roof / 1e12:.1f} TFLOP/s",
            xy=(x[1], roof / 1e9),
            xytext=(-6, 4),
    ...
    ax.annotate(
        f"HBM2e {peaks['bw'] / 1e12:.2f} TB/s",
        xy=(ridge / 6, ridge / 6 * peaks["bw"] / 1e9),
        fontsize=8,
        color="0.25",
        rotation=38,
    ...
    print(
        f"ceilings: FP64 {peaks['fp64'] / 1e12:.2f} TFLOP/s, "
    ...
    for (label, kname, prec), pts in sorted(points.items()):
        ...
        print(
            f"  {label}: {kname} [{prec}] AI={ai:.3f} FLOP/B, "
