...
int main(int argc, char** argv) {
  ...
  try {
    ...
    const auto symmetric_bytes = std::max<std::size_t>(4U * max_total * sizeof(float), 1U << 20U);
    ...
    auto* device_send =
        static_cast<float*>(gpu_bench_oshmpi_space_malloc(space, max_total * sizeof(float)));
    auto* device_recv =
        static_cast<float*>(gpu_bench_oshmpi_space_malloc(space, max_total * sizeof(float)));
    ...
    for (const auto count : message_sizes) {
      ...
      const auto stats = gpu_bench::run_benchmark(warmup, iterations, [&]() {
        ...
        for (int dst = 0; dst < pes; ++dst) {
          ...
          if (dst == pe) {
            check_cuda(cudaMemcpy(dest, src, block_bytes, cudaMemcpyDeviceToDevice),
                       "cudaMemcpy(self)");
          } else {
