...
// NOTE: this uses oneCCL point-to-point (ccl::send / ccl::recv) for a true halo
// exchange. The UNISA NCCL-enabled oneCCL fork does not implement every
// primitive (e.g. broadcast); if pt2pt is unimplemented this binary reports a
// backend error -- a documented gap, consistent with sycl_oneccl_pingpong.
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (rank == 0) {
      ...
      report.name = "sycl_oneccl_halo_2d";
      report.n = side;
      report.ranks = ranks;
