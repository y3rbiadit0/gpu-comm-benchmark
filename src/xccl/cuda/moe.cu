    ...
    for (const auto routing : routing_cases) {
      ...
      const auto stats = gpu_bench::run_benchmark(warmup, iterations, [&]() {
        ...
        for (int peer = 0; peer < ranks; ++peer) {
          ...
          if (plan.send_counts[index] > 0) {
            check_nccl(ncclSend(device_send + plan.send_displacements[index], plan.send_counts[index], ncclFloat,
                                peer, comm, stream),
                       "ncclSend(dispatch)");
          }
          if (plan.recv_counts[index] > 0) {
            check_nccl(ncclRecv(device_dispatch + plan.recv_displacements[index], plan.recv_counts[index], ncclFloat,
                                peer, comm, stream),
                       "ncclRecv(dispatch)");
          }
        }
        check_nccl(ncclGroupEnd(), "ncclGroupEnd(dispatch)");

        check_nccl(ncclGroupStart(), "ncclGroupStart(combine)");
        for (int peer = 0; peer < ranks; ++peer) {
          const auto index = static_cast<std::size_t>(peer);
          if (plan.recv_counts[index] > 0) {
            check_nccl(ncclSend(device_dispatch + plan.recv_displacements[index], plan.recv_counts[index], ncclFloat,
                                peer, comm, stream),
                       "ncclSend(combine)");
          }
          if (plan.send_counts[index] > 0) {
            check_nccl(ncclRecv(device_combined + plan.send_displacements[index], plan.send_counts[index], ncclFloat,
                                peer, comm, stream),
                       "ncclRecv(combine)");
          }
        }
        check_nccl(ncclGroupEnd(), "ncclGroupEnd(combine)");
        check_cuda(cudaStreamSynchronize(stream), "cudaStreamSynchronize(moe)");
      });
