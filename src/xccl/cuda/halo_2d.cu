...
int main(int argc, char** argv) {
  ...
  try {
    ...
    const auto stats = comm_playground::run_benchmark(warmup, iterations, [&]() {
      if (local_cols > 0) {
        pack_column_kernel<<<grid1d, block1d, 0, stream>>>(old_field, send_west, side, width, 1U);
        pack_column_kernel<<<grid1d, block1d, 0, stream>>>(old_field, send_east, side, width, local_cols);
      }
      ...
      if (local_cols > 0) {
        unpack_column_kernel<<<grid1d, block1d, 0, stream>>>(old_field, recv_west, side, width, 0U);
        unpack_column_kernel<<<grid1d, block1d, 0, stream>>>(old_field, recv_east, side, width, local_cols + 1U);
        stencil_kernel<<<grid2d, block2d, 0, stream>>>(old_field, new_field, side, local_cols, width);
      }
