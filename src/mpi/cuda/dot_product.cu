...
int main(int argc, char** argv) {
  ...
  try {
    ...
    // (representable in double up to 2^53), giving a size-independent check.
    ...
    // The local partial is constant across iterations, so compute it once and
    // time only the cross-rank scalar allreduce (the communication under test).
    ...
    const auto stats = comm_playground::run_benchmark(warmup, iterations, [&]() {
      MPI_Allreduce(device_partial, device_result, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    });
    ...
    if (rank == 0) {
      ...
      report.name = "cuda_mpi_dot_product";
