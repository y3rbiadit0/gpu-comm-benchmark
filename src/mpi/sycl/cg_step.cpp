...
// CG iteration communication skeleton (see src/mpi/cuda/cg_step.cu): SpMV
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    float* p_field = sycl::malloc_device<float>(side * width, queue);
    float* q_field = sycl::malloc_device<float>(side * width, queue);
    ...
    queue.memset(p_field, 0, side * width * sizeof(float)).wait();
    queue.memset(q_field, 0, side * width * sizeof(float)).wait();
    ...
    if (local_cols > 0) {
      ...
      queue.copy(q_field, host_q.data(), side * width).wait();
